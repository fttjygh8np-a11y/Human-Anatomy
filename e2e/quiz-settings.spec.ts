...
test('ayarlar: tema ve yazı boyutu yeniden yüklemeden sonra korunur', async ({ page }) => {
  ...
  await panel.getByRole('combobox', { name: /Tema/ }).selectOption('light')
  ...
  await expect.poll(() => storedSettings(page)).toMatchObject({ theme: 'light', fontScale: 1.2 })
  ...
  await expect
    ...
    .toBe('1.2')
  await modeButton(page, 'Ayarlar').click()
  await expect(panel.getByRole('combobox', { name: /Tema/ })).toHaveValue('light')
